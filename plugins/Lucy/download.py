...
async def download_pinterest_video_command(client, message):
    try:
        if len(message.text.split(" ")) == 1:
            await message.reply_text("Veuillez fournir un lien Pinterest après la commande.")
            return
        ...
        if video_url:
            ...
            await message.reply_text("Aucune vidéo trouvée dans le lien Pinterest.")
    except Exception as e:
...
async def download_instagram_reel(client, message):
    try:
        if len(message.text.split(" ")) == 1:
            await message.reply_text("Veuillez fournir un lien Instagram après la commande.")
            return
        ...
        if response.status_code == 200:
            ...
            if "content" in data and len(data["content"]) > 0:
                ...
                await message.reply_text("Aucun contenu trouvé dans la réponse.")
        else:
            await message.reply_text(f"La requête a échoué avec le code de statut : {response.status_code}")
    except Exception as e:
        await message.reply_text("Une erreur s'est produite, veuillez réessayer plus tard.")
